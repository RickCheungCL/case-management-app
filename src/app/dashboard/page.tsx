...
export default function DashboardPage() {
  ...
  const filteredCases = filterStatus === 'all' 
    ? cases 
    : cases.filter(c => c.status.toLowerCase() === filterStatus);
        ...
        <div>
          ...
          {filteredCases.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCases.map((c) => (
                <div key={c.id} className="bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
                  <div className="p-5 space-y-3">
                    <div className="flex justify-between items-start">
                      <h3 className="font-bold text-lg text-gray-800">{c.customerName}</h3>
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        c.status.toLowerCase() === 'active' ? 'bg-green-100 text-green-800' : 
                        c.status.toLowerCase() === 'completed' ? 'bg-blue-100 text-blue-800' : 
                        c.status.toLowerCase() === 'new' ? 'bg-yellow-100 text-yellow-800' :
